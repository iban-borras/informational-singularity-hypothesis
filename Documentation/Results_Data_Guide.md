...
This is the legacy v1 results guide. For the current HSI v2 paper-support and Phase 3 freeze state, start with:
...
The v2 result stack lives under `results/hsi_v2/` and is intentionally excluded from git because it contains large generated artifacts. Use each run's `manifest.json`, `summary.json`, and `report.md` for recovery.
