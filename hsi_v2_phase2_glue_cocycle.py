#!/usr/bin/env python3
"""
HSI v2 Phase 2 Overlap-Glue Cocycle Pilot

Minimal non-parent-local follow-up to the Gate 2 pilots:
- takes ordered overlapping parent pairs at m=24 sharing a base overlap at m=20
- lifts each parent to its dominant two-step descendant at m=32
- checks whether overlap-compatibility survives the lift
"""
...
from v2.phase2.glue import build_glue_rows, render_console_summary, render_markdown_report


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the HSI v2 Gate 2 overlap-glue cocycle pilot from existing Phase 1 artifacts."
    )
...
def main() -> int:
    ...
    phase_print(
        "Preparing Gate 2 overlap-glue cocycle pilot",
    ...
    if unsupported:
        parser.error(
            "Selected runs do not contain the requested glue window/policies for: "
            + ", ".join(unsupported)
        )

    phase_print(
        "Analyzing overlap-glue cocycle",
        f"{len(selected)} runs | policies={','.join(pilot_policies)}",
        quiet=args.quiet,
    )
    rows = build_glue_rows(selected, window_scales=window_scales, policies=pilot_policies)
    ...
    dataset_payload = {
        "stage": "phase2_glue_cocycle",
        "generated_at": generated_at,
        "selection": selection,
        "rows": rows,
        "notes": [
            "This pilot measures whether overlap-compatibility between parent-scale patterns survives dominant two-step lift.",
            "candidate pair = ordered parent patterns whose suffix(base_scale) matches the other prefix(base_scale).",
            "glue defect = dominant descendants fail the corresponding overlap match at the high scale.",
        ],
    }
    summary_payload = {
        "generated_at": generated_at,
        ...
        "compact_rows": [
            {
                ...
                "prefix_glueable_pair_fraction": row.get("prefix_glueable_pair_fraction"),
                "suffix_glueable_pair_fraction": row.get("suffix_glueable_pair_fraction"),
                "prefix_glue_defect_rate": row.get("prefix_glue_defect_rate"),
                "suffix_glue_defect_rate": row.get("suffix_glue_defect_rate"),
                "prefix_glue_defect_mass_rate": row.get("prefix_glue_defect_mass_rate"),
                "suffix_glue_defect_mass_rate": row.get("suffix_glue_defect_mass_rate"),
                "prefix_mean_glue_distance": row.get("prefix_mean_glue_distance"),
                "suffix_mean_glue_distance": row.get("suffix_mean_glue_distance"),
            }
            for row in rows
        ],
    }
    manifest_payload = {
        "run_slug": run_slug,
        "generated_at": generated_at,
        "script": "hsi_v2_phase2_glue_cocycle.py",
        "cwd": str(Path.cwd()),
        "outputs": {
...
def build_run_slug(selection: dict, timestamp: str) -> str:
    ...
    return (
        f"phase2-glue-cocycle__{iter_part}__seg-{selection['num_segments']}x{compact_int(selection['segment_bits'])}"
...
def _supports_window(run: dict, *, window_scales: tuple[int, int, int, int], policies: list[str]) -> bool:
