#!/usr/bin/env python3
"""HSI v2 Phase 4 P4-04 forward regime forecast.

This launcher turns the P4-03/P4-03b regime map into a small forward
prediction. It does not tune new bands after seeing the readout: the negative
channel is expected to remain in late reactivation for three new forward bands,
while the positive channel is expected to remain non-universal.
"""
...
DEFAULT_OUTPUT_DIR = "results/hsi_v2/phase4/p4_04_forward_regime_forecast"
DEFAULT_SEEDS = "607,709,811,907,1009,1103"
DEFAULT_LAGS = "-29500000,-27000000,-26500000,-24000000,24500000,26500000"

FORECAST_BANDS = (
    {
        "band_start_bits": 768_000_000,
        "expected_negative_regime": "late-reactivation",
        "reason": "first forward band after the P4-03b 759M late-reactivation holdout",
    },
    {
        "band_start_bits": 777_000_000,
        "expected_negative_regime": "late-reactivation",
        "reason": "second forward band tests persistence rather than a single isolated rebound",
    },
    {
        "band_start_bits": 786_000_000,
        "expected_negative_regime": "late-reactivation",
        "reason": "third forward band tests whether the reactivation survives another full band step",
    },
)
...
NEGATIVE_SUPPORT_THRESHOLD = 2


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run P4-04: a predeclared forward forecast of the late-reactivation "
            "regime under phase-matched-LZ and block-entropy pressure."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_slug = f"phase4-p4-04-forward-regime-forecast__bands-3__lags-6__{timestamp}"
    ...
    try:
        phase_print(
            "Preparing P4-04 forward regime forecast",
            (
                "bands=768M,777M,786M | expected_negative=late-reactivation | "
        ...
        write_outputs(run_dir, state)
        print(f"[x] P4-04 failed after {format_time(time.perf_counter() - started)}")
        ...
        state["error"] = str(exc)
        write_outputs(run_dir, state)
        print(f"[x] P4-04 failed after {format_time(time.perf_counter() - started)}: {exc}")
        print(f"Saved failure summary to: {run_dir / 'summary.json'}")
        return 1
...
def build_tests(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    tests = [
        {
            "test": "negative_forward_reactivation",
            "metric": "hits/total",
            ...
            "detail": (
                "At least two of three forward negative bands must remain strong "
...
def build_verdict(tests: list[dict[str, Any]]) -> dict[str, str]:
    ...
    if supported == len(tests):
        status = "forward-regime-forecast-supported"
    elif supported:
        status = "forward-regime-forecast-mixed"
    else:
        status = "forward-regime-forecast-rejected"
    return {
        "status": status,
        "short_reading": (
            "P4-04 tests whether the late negative-channel reactivation observed "
            "near 759M persists into three untouched forward bands under the same "
            "hard null envelope."
        ),
...
def render_console_summary(state: dict[str, Any]) -> str:
    lines = [
        "Phase 4 P4-04 forward regime forecast",
        "-" * 108,
    ]
...
def render_report(state: dict[str, Any], run_dir: Path) -> str:
    lines = [
        "# HSI v2 Phase 4 P4-04 forward regime forecast",
        "",
        "P4-04 is a predeclared forward forecast, not a boundary micro-map.",
        "It tests whether the late negative-channel reactivation continues into three untouched bands.",
        "",
        "## Parameters",
        "",
