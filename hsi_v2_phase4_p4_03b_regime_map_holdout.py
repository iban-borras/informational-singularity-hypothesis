#!/usr/bin/env python3
"""HSI v2 Phase 4 P4-03b piecewise-regime holdout.

This launcher tests the P4-03 regime map on bands that were not used to
define the map. It runs a compact lag grid against the phase-matched-LZ and
block-entropy null axes, then classifies each holdout band as active, shoulder,
shadow, or reactivation.
"""
...
DEFAULT_OUTPUT_DIR = "results/hsi_v2/phase4/p4_03b_regime_map_holdout"
DEFAULT_SEEDS = "607,709,811,907,1009,1103"
DEFAULT_LAGS = "-29500000,-27000000,-26500000,-24000000,24500000,26500000"

HOLDOUT_BANDS = (
    {
        "band_start_bits": 702_000_000,
        "expected_negative_regime": "active-core",
        "reason": "interior interpolation between 696M and 705M active-core anchors",
    },
    {
        "band_start_bits": 711_000_000,
        "expected_negative_regime": "active-core",
        "reason": "interior interpolation between 705M and 714M active-core anchors",
    },
    {
        "band_start_bits": 732_000_000,
        "expected_negative_regime": "pre-shadow-shoulder",
        "reason": "out-of-sample shoulder between 729M pre-shadow and 738M shadow",
    },
    {
        "band_start_bits": 741_000_000,
        "expected_negative_regime": "null-shadow-boundary",
        "reason": "out-of-sample interior of the 738M-747M null-shadow zone",
    },
    {
        "band_start_bits": 759_000_000,
        "expected_negative_regime": "late-reactivation",
        "reason": "late extension after the 756M reactivation anchor",
    },
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run P4-03b: an out-of-sample holdout test of the P4-03 "
            "piecewise-regime map."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_slug = f"phase4-p4-03b-regime-map-holdout__bands-5__lags-6__{timestamp}"
    ...
    try:
        phase_print(
            "Preparing P4-03b regime-map holdout",
            (
                "bands=702M,711M,732M,741M,759M | lags=6 | "
                "nulls=phase-matched-lz,block-entropy"
            ),
            quiet=args.quiet,
        )
        flush_output()
        ...
        holdout_rows = classify_holdouts(lag_rows)
        tests = build_tests(holdout_rows)
        ...
        print(f"\nSaved summary to: {run_dir / 'summary.json'}")
        print(f"Saved report to: {run_dir / 'report.md'}")
        print(f"Saved holdout CSV to: {run_dir / 'holdout_regimes.csv'}")
        print(f"Saved tests CSV to: {run_dir / 'holdout_tests.csv'}")
        print(f"Saved manifest to: {run_dir / 'manifest.json'}")
        return 0
        ...
        write_outputs(run_dir, state)
        print(f"[x] P4-03b failed after {format_time(time.perf_counter() - started)}")
        ...
        state["error"] = str(exc)
        write_outputs(run_dir, state)
        print(f"[x] P4-03b failed after {format_time(time.perf_counter() - started)}: {exc}")
        print(f"Saved failure summary to: {run_dir / 'summary.json'}")
        return 1


def build_lag_response_command(args: argparse.Namespace, phase1_dir: Path, run_dir: Path) -> list[str]:
    band_starts = ",".join(str(row["band_start_bits"]) for row in HOLDOUT_BANDS)
    return [
        sys.executable,
...
def classify_holdouts(lag_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    by_band: dict[str, list[dict[str, str]]] = {}
    for row in lag_rows:
        by_band.setdefault(row["band"], []).append(row)

    rows: list[dict[str, Any]] = []
    for band_spec in HOLDOUT_BANDS:
...
def expected_hit(channel: str, expected: str, observed: str, strong_count: int) -> bool:
    ...
    if expected == "active-core":
        return observed == "strong-channel"
    if expected == "pre-shadow-shoulder":
        return observed == "pre-shadow-shoulder"
    if expected == "null-shadow-boundary":
        return observed == "null-shadow-boundary"
...
def build_tests(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    tests = [
        {
            "test": "negative_holdout_regime_hits",
            "metric": "hits/total",
            "value": f"{sum(1 for row in negative if row['prediction_hit'])}/{len(negative)}",
            "threshold": "5/5",
            "verdict": "supported" if all(row["prediction_hit"] for row in negative) else "rejected",
            "detail": "All negative holdout bands must match their preassigned regime class.",
        },
        {
            "test": "positive_channel_nonuniversality",
            "metric": "nonuniversal_positive",
            "value": f"{sum(1 for row in positive if row['observed_regime'] == 'strong-channel')}/{len(positive)} strong",
            "threshold": "not all strong",
            "verdict": (
                "supported"
                if 0 <= sum(1 for row in positive if row["observed_regime"] == "strong-channel") < len(positive)
                else "rejected"
            ),
            "detail": "Positive holdout channels should remain intermittent or absent, not universal.",
        },
    ]
    return tests
...
def build_verdict(tests: list[dict[str, Any]]) -> dict[str, str]:
    if all(test["verdict"] == "supported" for test in tests):
        status = "holdout-regime-map-supported"
    elif any(test["verdict"] == "supported" for test in tests):
        status = "holdout-regime-map-mixed"
    else:
        status = "holdout-regime-map-rejected"
    return {
        "status": status,
        "short_reading": (
            "P4-03b tests whether the P4-03 piecewise-regime map survives new "
            "bands under both phase-matched-LZ and local transition-entropy pressure."
        ),
        "paper_boundary": "Internal Phase 4 holdout only; no manuscript edit is authorized.",
    }


def render_console_summary(state: dict[str, Any]) -> str:
    lines = [
        "Phase 4 P4-03b piecewise-regime holdout",
        "-" * 108,
    ]
...
def render_report(state: dict[str, Any], run_dir: Path) -> str:
    lines = [
        "# HSI v2 Phase 4 P4-03b piecewise-regime holdout",
        "",
        "P4-03b is an out-of-sample test of the P4-03 regime map.",
        "It uses new band starts and keeps the lag grid compact.",
        "",
        "## Parameters",
        "",
    ...
    lines.extend(
        [
            ...
            "- Holdout regimes: `holdout_regimes.csv`",
            "- Holdout tests: `holdout_tests.csv`",
    ...
    write_json(
        run_dir / "manifest.json",
        {
            ...
            "outputs": {
                ...
                "holdout_regimes": str(run_dir / "holdout_regimes.csv"),
                "holdout_tests": str(run_dir / "holdout_tests.csv"),
                "manifest": str(run_dir / "manifest.json"),
            },
        },
    )
    write_csv(run_dir / "holdout_regimes.csv", state.get("holdout_rows", []), HOLDOUT_FIELDS)
    write_csv(run_dir / "holdout_tests.csv", state.get("tests", []), TEST_FIELDS)
